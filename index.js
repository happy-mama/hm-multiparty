...
var FILENAME_PARAM_RE =
  /\bfilename=(?:"(.*?)"|([!#$%&'*+.0-9A-Z^_`a-z|~-]+))($|; )/i
...
function Form(options) {
  ...
  self.prefixDir = options.prefixDir ? validatePrefixDir(options.prefixDir) : ''
  ...
  for (i = 0; i < len; i++) {
    ...
    switch (state) {
        ...
        if (index === boundaryLength - 2 && c === HYPHEN) {
          ...
          if (c !== CR)
            return self.handleError(
              createError(400, 'Expected CR Received ' + c)
            )
          index++
          break
          ...
          if (c !== LF)
            return self.handleError(
              createError(400, 'Expected LF Received ' + c)
            )
          index = 0
      ...
      case HEADER_FIELD:
        ...
        if (cl < A || cl > Z) {
          self.handleError(
            createError(400, 'Expected alphabetic character, received ' + c)
          )
          return
        }
        break
      ...
      case CLOSE_BOUNDARY:
        if (c !== HYPHEN)
          return self.handleError(
            createError(400, 'Expected HYPHEN Received ' + c)
          )
        if (index === 1) {
          self.onParsePartEnd()
          state = END
...
Form.prototype.onParseHeadersEnd = function (offset) {
  ...
  switch (self.partTransferEncoding) {
    ...
    default:
      return createError(
        400,
        'unknown transfer-encoding: ' + self.partTransferEncoding
      )
  }
  ...
  self.destStream.byteCount = partContentLength
    ...
    : self.bytesExpected
    ? self.bytesExpected -
      self.destStream.byteOffset -
      self.boundary.length -
      LAST_BOUNDARY_SUFFIX_LEN
    : undefined
...
LimitStream.prototype._transform = function _transform(
  chunk,
  encoding,
  callback
) {
...
function handleFile(self, fileStream) {
  ...
  var publicFile = {
    ...
    path: uploadPath(self.uploadDir, self.prefixDir, fileStream.filename),
  ...
  var internalFile = {
    ...
    ls: null,
    ws: fs.createWriteStream(publicFile.path, { flags: 'w+' })
  }
...
function handleField(self, fieldStream) {
  ...
  fieldStream.on('readable', function () {
    ...
    if (self.totalFieldSize > self.maxFieldsSize) {
      self.handleError(
        createError(413, 'maxFieldsSize ' + self.maxFieldsSize + ' exceeded')
      )
      return
    }
...
function validatePrefixDir(prefixDir, self) {
  var matched = prefixDir.match(/\/[\w+\/]+/g)

  if (!matched || matched.length > 1) {
    throw Error('prefixDir is incorrect')
  }

  return prefixDir
}

function uploadPath(baseDir, prefixDir, filename) {
  return path.join(baseDir, prefixDir, filename)
}
